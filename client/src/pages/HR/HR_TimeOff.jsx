...
export default function HR_TimeOff() {
  ...
  const fetchLeaveData = async () => {
    setLoading(true)
    try {
      console.log('Fetching HR leave data with filters:', filters)
      
      const params = new URLSearchParams()
      if (filters.leaveType) params.append('leaveType', filters.leaveType)
      if (filters.status) params.append('status', filters.status)
      
      const response = await api.get(`/hr/timeoff?${params.toString()}`)
      
      console.log('HR Leave API full response:', response)
      ...
      console.error('Error fetching HR leave data:', error)
  ...
  const handleApprove = async (id) => {
    try {
      await api.patch(`/hr/timeoff/${id}/status`, { status: 'approved' })
  ...
  const handleReject = async (id) => {
    try {
      await api.patch(`/hr/timeoff/${id}/status`, { status: 'rejected' })
  ...
  return (
    <div className="min-h-screen space-y-6">
      ...
      <section className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-green-600 to-emerald-600 text-white shadow-xl p-8">
        <div className="relative z-10 flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-4 mb-1">
              <h1 className="text-4xl font-bold">Time Off Overview</h1>
            ...
            <p className="text-green-100 text-base">Manage and review employee leave requests</p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white rounded-xl shadow-md border border-gray-200 p-5">
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <select
            ...
            className="px-4 py-2 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all"
          >
      ...
      <section className="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
        <div className="p-6 border-b border-gray-200 bg-gray-50">
          <h2 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
            <FaUmbrellaBeach className="text-green-500" /> Daily Attendance Records
          </h2>
        </div>
